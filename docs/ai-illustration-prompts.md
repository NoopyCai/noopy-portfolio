...
| `public/cabin/cabin-front.png` | 1672×941 RGBA · 63 KB | 立柱 ×2 + 頂端橫杆的去背層 | **已接進 DOM**（階段 1，2026-08-14） |
...
### 立柱層接進 DOM（階段 1，2026-08-14）

上線時把這一層從 2715×1528 重採樣成 **1672×941**（`lanczos3`、`fit: fill`，PNG effort 10 → 119 KB 降到 63 KB）。理由：

- 它的母檔 `docs/assets/cabin-front-green.png` 本來就是 1672×941，2715 版是**放大出來的**，沒有任何額外細節（幾何實測完全一致：橫杆 x 0.42–99.52%、立柱 y 4.46–96.60%、柱身 20.22–21.59% 與 78.29–79.61%，alpha 最大值仍是 255）。
- ② 版基底已經是 1672×941，前景比基底細只是讓瀏覽器每幀多解一張 4.1 M 像素的圖。實測（1440×900、4× CPU throttle、SwiftShader）換成同尺寸後每幀成本降了約一半：p50 對階段 0 的落差從 +32% 收到 +21%，整段 ride 的總時間從 +20% 收到 +9%。
- 重採樣走預乘 alpha，全透明像素的中性鋼灰 RGB `(76,105,113)` 保留；只有 407 個邊緣像素在 alpha ≤ 40 時 RGB 落到近黑（貢獻 < 16%，量不出來也看不出來）。

原始的 2715 版備份在 `/tmp/cabin-front-2715-backup.png`（不入庫，母檔還在 `docs/assets/`，隨時可以重出）。
