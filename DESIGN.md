...
- **車門過場**（gate → ride）：真的 3D 場景（three.js，`components/door3d/`）。四拍：門縫細光 → 塞拉門 3D 滑開 + 暖光楔灑月台地面 → 相機 dolly-in 穿過門框（門柱掠過＝視差）→ 相機定住。全程由捲動進度插值，可倒放（倒捲就倒關）。門板、警戒條、光楔、門縫光**全是程序繪製的 canvas 貼圖**，沒有外來素材；three 走 `dynamic import()` 的 async chunk，首頁 First Load JS 不受影響。
- **門開完不落幕**（L2a）：相機停在車廂裡，同一個場景繼續當 `ride` 的舞台——車廂牆（三個窗區是**真的洞**）、立柱、窗景遠近層、月台層、隧道掃光全在場景裡。**沒有交棒**：doorP 0.99 與 1.01 的畫面逐像素相同。DOM 只剩「永遠不進 WebGL」的東西：跑馬燈（文字）、玻璃反光、資訊卡、路線圖。沒有 WebGL 的機器靜默降級成精簡 DOM 車廂（無隧道／月台層／門過場）。
- **出站一鏡到底**（L2b）：`exit` 相位整段由**場景相機**演，不再有 CSS 的假 3D（`rotateY(-85deg)` 那組已刪）。五拍：窗景層深度塌陷（相機不動）→ 起身（升高 + 俯角補償）→ 半拍 → 轉身（繞牆面樞紐公轉，立柱與牆真的錯開＝真視差）→ 退出門洞、門在眼前關上。全程由 `e` 插值，倒捲就是倒著演（門重開、轉回、坐下）。唯一保留的 DOM 交棒是最後 Concourse hero 疊上來——它自帶不透明底色，沒有對位問題。手機不再有 2.5D 特例（構圖依 `camera.aspect` 連續調整）。沒有 WebGL 時退回「起身 + 淡出」。
- **滑鼠視差**：sway 層 `translate3d(±15px, ±12px) rotateX/Y(±1.4deg) scale(1.035)`，只在 `ride` 生效，`gate`/`exit` 平滑收斂回 0。`scale(1.035)` 是過掃描，**只有車廂進這層**（文字會被重新取樣而發糊）。3D 場景的 canvas 走同一組位移與旋轉、但**不吃那個 scale**：它自己的盒子就大 3.5%（縮放一張已光柵化的點陣圖會多一次重取樣，小字會軟掉）。WebGL 因此在滑鼠視差期間一幀都不用重畫——合成器搬已經畫好的幀。
