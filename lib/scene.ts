...
export function drawScene(canvas: HTMLCanvasElement, type: SceneType, opts: { bg?: boolean } = {}) {
  ...
  if (type === "platform") {
    ...
    for (const [cx, cy] of [[54, 12], [128, 20], [176, 9], [92, 6]] as number[][]) cloud(cx, cy, "#f6fafd");
    ridge(42, 68, "#9cc0aa", 6, 4, 2.2); // 最遠山(最亮)
    ridge(50, 68, "#7fa890", 5, 3, 0.2); // 遠山
    ridge(58, 68, "#5f8a72", 4, 2, 1.4); // 近山
    skyline(78, 10, 26, "#8a94a0", false, true); // 遠樓+屋頂
    skyline(106, 16, 44, "#7d8791", true, true); // 密集城市+屋頂(亮窗暖白)
    R(0, 106, W, H, "#3a3f45"); R(0, 106, W, 2, "#4a4f55"); for (let x = 0; x < W; x += 14) R(x, 114, 7, 2, "#d8d2b8"); // 街道
    for (let x = 4; x < W; x += 9) R(x, 108, 2, 1, ["#e8e2c0", "#c94a3a"][(x / 9 | 0) % 2]); // 車流燈(可重複)
    if (!bg) {
      disc(30, 16, 6, "#fff6d8", 9); // 太陽
      for (const [bx, by] of [[68, 24], [150, 30]] as number[][]) { R(bx, by, 3, 1, "#5f8a72"); R(bx + 2, by, 3, 1, "#5f8a72"); } // 飛鳥
      const tx = 128; // 台北 101
    ...
    for (let x = 8; x < W; x += 46) { R(x, 73, 4, 5, "#1b2716"); R(x + 5, 75, 3, 3, "#1b2716"); R(x + 6, 76, 1, 1, "#ffcf7a"); } // 遠方村落(山腳,可重複)
    ...
    for (let x = 26; x < W; x += 54) { R(x, 68, 1, 20, "#0e160a"); R(x - 3, 70, 7, 1, "#0e160a"); } // 電線桿+橫擔(可重複)
    for (let x = 26; x < W; x += 54) for (let sx = x; sx < x + 54 && sx < W; sx++) { const y = (71 + (1 - Math.cos((sx - x) / 54 * Math.PI * 2)) * 2) | 0; R(sx, y, 1, 1, "#0e160a"); } // 電線
    for (let i = 0; i < 6; i++) R((rnd() * W) | 0, 90 + ((rnd() * 22) | 0), 1, 1, "#e8eef0"); // 稻田白鷺(可重複)
    ...
    if (!bg) {
      ...
      R(60, 80, 1, 10, "#3a2a18"); R(56, 82, 9, 1, "#3a2a18"); disc(60, 78, 2, "#7a5a3a", 0); R(58, 79, 1, 1, "#0a0a0a"); R(61, 79, 1, 1, "#0a0a0a"); // 稻草人
    }
  } else if (type === "sea") {
