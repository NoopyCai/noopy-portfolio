...
// A3 深度層:同一站的窗景拆成遠近兩張,中央窗以不同的平移速度疊起來(遠山慢、近景快)。
//
//   layer 不傳  = 現行完整版(左右窗、月台層、任何舊呼叫端都走這條,像素不變)
//   layer "far" = 天空 + 星 + 雲 + 遠山 + 中景剪影
//   layer "near"= 近景建物 / 地面 / 前景物件 + **全部的地標**
//
// 地標(每個場景末端 `if (!bg)` 的那一組)為什麼一定在 near:它「每站出現一次」的機制是
// blit 的三段長條 [bg | full | bg] 加上每站剛好走一圈的 pan。far 層只走 0.35 圈,地標放進去
// 就會在隨機的站別隨機出現(第 0 站甚至永遠看不到)。所以 far 恆等於「可無縫平鋪的背景」,
// 它的 bg / full 是同一張圖 —— 快取因此是每站 3 張(far / near-bg / near-full)而不是 4 張。
//
// 兩層是**同一組 draw call 的分割**(不是重畫兩次):以相同 pan 疊起來就等於原本的完整版,
// 差速才是 A3 的全部內容。分割點一律落在「地面填滿」那一句之前 —— 近層的地面是不透明的,
// 蓋住遠層的下緣,所以遠層不需要補畫背景。
export type SceneLayerKind = "far" | "near";

export function drawScene(canvas: HTMLCanvasElement, type: SceneType, opts: { bg?: boolean; layer?: SceneLayerKind } = {}) {
  const bg = !!opts.bg;
  // 不傳 layer = 兩邊都畫 = 完整版(向後相容)
  const far = opts.layer !== "near";
  const near = opts.layer !== "far";
  ...
  if (type === "platform") {
    // 月台**不拆層**:它是站內場景,對向牆與立柱在同一個屋簷下,差速讀起來是「牆在飄」
    // 而不是深度;而站名燈牌(台北)是這一站的地標,放進只走 0.35 圈的 far 層就會失蹤。
    // 整組留在 far,near 是空的 —— Window 也不會替月台要 layer(見 SPLIT 判斷)。
    if (!far) return;
    ...
    if (near) {
      ...
      // 橋是這一站離車最近的量體,和水面、倒影同層 —— 拆層之後它掠過遠岸的速度才是對的。
    ...
    if (near) {
      ...
      if (!bg) {
        // 101 的基座與近景城市同一條街,所以它在 near —— 天際線動、101 跟著動,不會滑開
    ...
    if (far) {
      ...
      // 零星燈火跟著村落留在 far:它們畫在山腰上,放進 near 就會變成「燈在山上滑動」
